...
**Corroboration gate — the whole reason this is a distinct, governed command rather than a bare results file.** Each result must carry `source_name`/`source_url`/`retrieved_at_utc`/`evidence_mode`/`evidence_hash`/`authoritative`/`reported_score` for one or more sources (see `src/pcbf_calculator/orchestration/schemas/manual_results_input.v1.schema.json` for the full documented shape). A result is accepted only when at least one source is marked `authoritative` and agrees with the claimed score, **or** at least two sources with distinct `source_name` values agree. Any source disagreeing with the claimed score fails the whole result closed (`SETTLE_SOURCE_SCORE_DISAGREEMENT`) — never averaged or voted on. Fewer than the required corroboration is `SETTLE_INSUFFICIENT_CORROBORATION`, never accepted on trust.

**Operator-attested evidence — a deliberate, explicit exception, never a silent one.** `evidence_mode` is either `"ARCHIVED"` (the ordinary case: a real `evidence_hash` of independently-archived bytes, no `operator_attestation`) or `"OPERATOR_ATTESTED"` (`evidence_hash` is `null` — never a sentinel string dressed up to look like a hash — and `operator_attestation` records who explicitly accepted this source without one, when, why, and which exact source artifact, by its own tool-computed SHA-256, the claim came from). This exists for exactly one situation: a source's page genuinely cannot be archived (e.g. an outbound network policy blocks the host), evidence *construction* otherwise passed the ordinary corroboration rule, and a human operator has reviewed and explicitly accepted that gap rather than waiting indefinitely. It is never accepted on the file's own say-so alone: every `OPERATOR_ATTESTED` source is rejected (`SETTLE_OPERATOR_ATTESTED_NOT_ACCEPTED`) unless the invocation *also* passes `--accept-operator-attested-evidence-for SOURCE_ARTIFACT_SHA256`, naming the exact hash the source's own `operator_attestation.source_artifact_sha256` must match (`SETTLE_OPERATOR_ATTESTATION_SOURCE_ARTIFACT_MISMATCH` otherwise) — two independent human gates, not one. A shape inconsistency (an `ARCHIVED` source carrying an `operator_attestation`, or an `OPERATOR_ATTESTED` source carrying a real-looking hash) is rejected outright (`SETTLE_EVIDENCE_MODE_INCONSISTENT`). A single result row mixing an `ARCHIVED` source with an `OPERATOR_ATTESTED` one is explicitly supported. Without `--accept-operator-attested-evidence-for`, a file containing only `ARCHIVED` sources behaves exactly as before this feature existed — no weakening of the ordinary path. A forecast already `SCORED` from operator-attested evidence is exactly as immutable as one scored any other way: a later, successfully archived re-submission of the same fixture is a duplicate-skip or a real conflict, never a silent rewrite.
...
python -m pcbf_calculator convert-manual-results-evidence \
  ...
  [--archive-manifest ledger_data/manual_evidence/archive-manifest.json]
```

Read-only against the ledger (a `fixture_id` lookup only) — never writes a `SCORED` event, never touches `--confirm` at all. For each `result_status: "VERIFIED"` row: looks up its `fixture_id` in the CURRENT forecast-ledger state (no match is `SETTLE_FIXTURE_NOT_IN_LEDGER`; more than one is `SETTLE_FIXTURE_ID_AMBIGUOUS`; no full settlement identity recorded is `SETTLE_NO_SETTLEMENT_IDENTITY`); independently re-resolves the claimed competition/teams through `identity.py` and compares against what the ledger already recorded for that `fixture_id` (`SETTLE_IDENTITY_MISMATCH` on disagreement).

**Never trusts an `evidence_sha256`/`retrieved_at_utc` the evidence-collection file itself supplies — only a trusted archive manifest, independently re-verified.** Those two fields are read from the untrusted input for nothing: an LLM that fabricates an `evidence_summary` could just as easily fabricate a plausible-looking hash to match it, and internal consistency between two fabricated values proves nothing. Instead, each source's own `source_url` is looked up in a separate, trusted **archive manifest** (`schemas/manual_evidence_archive_manifest.v1.schema.json`, default location `<ledger-dir>/manual_evidence/archive-manifest.json` — produced by `archive-manual-results-evidence`, below, the evidence-preservation tool that actually fetches and archives the cited page; never by this command and never by an LLM). No manifest entry for that URL is `SETTLE_EVIDENCE_NOT_ARCHIVED` — until that source has actually been archived, this is its outcome, and that is the correct result, not a bug to route around. When an entry does exist, this command reads the actual archived bytes at the manifest's own `archive_path` and **recomputes their SHA-256 itself** — only a match against the manifest's own recorded hash is accepted (`SETTLE_ARCHIVE_INTEGRITY_MISMATCH` otherwise, e.g. a tampered or corrupted archive file). The `evidence_hash`/`retrieved_at_utc` this command ultimately writes come from that independently-verified value alone. Each surviving source's own `evidence_summary` text is still independently parsed and must actually support the row's claimed score (`SETTLE_EVIDENCE_SCORE_MISMATCH` otherwise) — never trusted just because a row says `VERIFIED`. `source_type` maps to `authoritative` via a small, fixed allowlist (`OFFICIAL_CLUB`/`OFFICIAL_COMPETITION`/`NEWSWIRE`/`MAJOR_PUBLICATION`; anything else, including `OTHER_CREDIBLE`, is not). Surviving sources are run through `ingest-manual-results`'s own corroboration function, reused verbatim — a row this converts is guaranteed to also pass that command's own gate.

**Operator-attested fallback — off by default, explicit every time.** When a source fails *only* because it was never archived (no manifest entry for its `source_url` at all — never an integrity mismatch, never an unparseable/mismatched summary, which are always hard rejects regardless of this flag), `--accept-operator-attested-evidence` (requires `--accepted-by NAME` too) emits it as `evidence_mode: "OPERATOR_ATTESTED"` instead of rejecting it — but only after its `evidence_summary` still independently parses and matches the claimed score, exactly like an `ARCHIVED` source must. `operator_attestation.source_artifact_sha256`/`source_artifact_name` are computed by this command from the real bytes of the evidence-collection file being read — never taken from the file's own content. `--attestation-reason-code` (default `ARCHIVE_UNVERIFIABLE_EGRESS`) is recorded verbatim. This changes nothing about the ordinary path: without the flag, behavior is identical to before this existed. `ingest-manual-results` still requires its own separate `--accept-operator-attested-evidence-for` flag before treating any of this as accepted — this converter's flag only controls whether it is willing to construct such a row at all.

Writes `converted-manual-results-input.json` (ready to hand to `ingest-manual-results`, still dry-run there by default), `conversion-report.json` (also reports `archive_manifest`/`archive_manifest_entries`), and `conversion-rejected.json` (every rejected row with its typed reason). Its output is advisory only — nothing here is authoritative until `ingest-manual-results` itself, separately, accepts and (with `--confirm`) commits it.

### Evidence preservation — the trusted archiver (`archive-manual-results-evidence`)

The tool `convert-manual-results-evidence`'s own archive-manifest lookup depends on: fetches the real page a source cites, verifies it actually supports the claimed result, and appends one tool-generated entry to the manifest. The only command in this codebase that makes an outbound network request — every other command stays offline.
...
**Every manifest field is tool-generated at fetch time, never copied from the evidence-collection file.** `retrieved_at_utc`, `http_status`, `content_type`, `byte_count`, and `sha256` all come from a real HTTPS response and the real bytes written to disk; a source's own `evidence_sha256`/`retrieved_at_utc` are never read by this command at all, for the identical reason `convert-manual-results-evidence` never reads them either.

**SSRF-safe by construction.** Only an `https` URL is ever fetched. Before connecting, and before following any redirect hop, the candidate hostname is resolved and every resolved address is checked against `ipaddress`'s own private/loopback/link-local/multicast/reserved/unspecified classifications — a destination that resolves to any of those is refused, whether it is the URL's own host or a redirect target reached from it. Redirects are capped at 5.

**Bounded fetch.** A fixed timeout, a fixed maximum download size (abandoned mid-stream the instant it would be exceeded, never buffered unbounded first), and a fixed allowed `Content-Type` set (`text/html`/`application/xhtml+xml`/`text/plain` — never a binary or media type).

**Content is independently validated, never assumed from a 200 status alone.** The archived page's own visible text (scripts/style/markup stripped) must actually contain the claimed home team, the claimed away team, the claimed scoreline, and at least one completed-match marker (e.g. "full time", "final score") — a page that never mentions the match, or reads as upcoming rather than finished, is refused (`ARCHIVE_CONTENT_DOES_NOT_SUPPORT_RESULT`). A page whose visible text is implausibly short after stripping markup — the common signature of a JavaScript-only single-page app that never server-rendered anything — is refused first, as `ARCHIVE_JS_ONLY_OR_EMPTY`. When `--ledger-dir` is supplied and the cited `fixture_id` already has a RECORDED forecast with a `scheduled_date`, that date's year is additionally required to appear in the text. **`evidence_summary` is never read by this command at all** — content validation uses only the row's own structured `participants`/`home_score`/`away_score` fields, checked against the real page, never the LLM's prose about it.

**Append-only, idempotent, immutable.** A `requested_url` already in the manifest is skipped, never re-fetched — safe to re-run against the same evidence-collection file as new sources appear day to day. The whole manifest is rewritten atomically (temp file + rename) on every run that archives something new; an existing entry is never edited or removed. Archived bytes are written once, at a content-addressed path (`<sha256>.bin`) that is never opened for writing again once it exists.

Writes `archive-report.json` (counts: rows seen, sources attempted/archived/already-archived/rejected) and `archive-rejected.json` (every rejected source with its typed `ARCHIVE_*` reason). Never touches `ingest-manual-results`, its `--confirm` gate, or the betting ledger; the one optional `--ledger-dir` read is a `fixture_id` lookup used only to strengthen content validation.
